package mg.vinaAkoho.vina_akoho.entity.produit;

import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

import java.math.BigDecimal;
import java.time.LocalDateTime;

@Entity
@Table(name = "categorie")
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
public class Categorie {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "libelle", nullable = false, unique = true, length = 100)
    private String libelle;

    @Column(name = "description", columnDefinition = "TEXT")
    private String description;

    @Column(name = "pourcentage_proteine", precision = 5, scale = 2)
    private BigDecimal pourcentageProteine;

    @Column(name = "pourcentage_matiere_grasses", precision = 5, scale = 2)
    private BigDecimal pourcentageMatiereGrasses;

    @Column(name = "pourcentage_humidite_max", precision = 5, scale = 2)
    private BigDecimal pourcentageHumiditeMax;

    @Column(name = "marge_pourcentage", precision = 5, scale = 2)
    private BigDecimal margePourcentage;

    @Column(name = "actif", nullable = false)
    private Boolean actif = true;  // ← AJOUT

    @Column(name = "created_at", updatable = false)
    private LocalDateTime createdAt;

    @Column(name = "updated_at")
    private LocalDateTime updatedAt;

    @PrePersist
    protected void onCreate() {
        this.createdAt = LocalDateTime.now();
        this.updatedAt = LocalDateTime.now();
        if (this.actif == null) {
            this.actif = true;
        }
    }

    @PreUpdate
    protected void onUpdate() {
        this.updatedAt = LocalDateTime.now();
    }
}