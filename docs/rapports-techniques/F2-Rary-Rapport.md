...
Module : Gestion des matières premières (T2.1 → T2.4)
Auteur : Rary
...
### PAMP = coût unitaire
Le prix d'achat est verrouillé à la création de la fiche (règle métier de l'écran entrée en stock). Tous les lots d'une même MP ont donc le même coût unitaire, ce qui rend le PAMP identique au `coutUnitaire`.
...
1. Un `lot_mp` avec `quantiteInitiale = quantiteRestante = quantite` reçue.
2. Un `mouvement_stock_mp` de type `ENTREE` pour la traçabilité (RG03/RG05).
